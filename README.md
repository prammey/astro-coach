...
submissions, up to 3 graded attempts per question, solution unlocking,
detailed topic analytics, an activity calendar with streaks, and a
Strengths panel.

## Progress: activity, streaks, strengths and badges

The dashboard shows progress worked out on the fly from a student's own
history (`UserAttempt`, `UserQuestionProgress`, scored `FrqSubmission`
rows), so there are no extra tables and past work counts straight away.
The rules are pure functions in `src/lib/progress/` with tests.

- **Activity calendar (Pro):** a GitHub-style year of days, brighter blue
  for more questions done. Hovering a day shows its MCQ and FRQ counts.
  Each question counts once per day, so retries don't inflate it. Days
  follow the student's own time zone. Current and longest streaks sit
  beside it; a streak only breaks once a whole day is missed.
- **Strengths (Pro):** one bar per topic, strongest first, coloured red →
  orange → yellow → green → blue as it fills; hovering shows plain
  accuracy and how many questions it rests on. The score
  (`src/lib/progress/strength.ts`) shrinks accuracy towards 50% until there
  is enough evidence, scales it by how much of the topic has been covered,
  then lifts the middle gently — so two lucky answers never outrank forty
  solid ones, and a bar only nears full with lots of accurate work. An FRQ
  counts as three MCQ answers of evidence, at its share of points.
- **Badges (every signed-in account):** 18 badges, grey until earned, with
  name, how to earn it and progress on hover. Definitions are in
  `src/lib/progress/badges.ts`; the art in `public/badges/` is generated by
  `python3 scripts/generate-badges.py`.
- **Free accounts** see a heavily blurred picture of made-up example data
  with a "Pro users only" lock at the bottom of the dashboard. The server
